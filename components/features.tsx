...
function CornerDoodle() {
  return (
    <svg
      aria-hidden="true"
      viewBox="0 0 80 80"
      className="absolute -top-4 -right-4 w-24 h-24 text-warmth-300 opacity-15 group-hover:opacity-35 transition-opacity duration-soft pointer-events-none"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
...
export function Features({ full = false }: FeaturesProps) {
  ...
  const featureIcons = [BarChart3, BookHeart, Wind, Users, ShieldAlert, Sparkles];
  ...
  return (
    <div className="relative space-y-6 md:space-y-12 flex flex-col items-center py-4 md:py-10 animate-fade-in">
      <div className="space-y-2 md:space-y-4 text-center max-w-2xl px-4 flex flex-col items-center">
        <h2 className="text-2xl-fluid @lg/card:text-3xl-fluid font-semibold tracking-tight text-foreground transition-colors duration-soft">
          {dict.features.title}
        </h2>
      </div>
      ...
      >
        {displayList.map((feature, index) => {
          ...
          return (
            <div
              key={index}
              className="group relative p-4 md:p-6 rounded-card bg-card border border-border/50 shadow-card transition-all duration-slow ease-out-soft hover:bg-warmth-50/40 hover:-translate-y-1.5 hover:shadow-card-hover overflow-hidden"
            >
              <CornerDoodle />
              <div className="relative z-10 space-y-2 md:space-y-4">
                <div className="w-9 h-9 md:w-12 md:h-12 flex items-center justify-center rounded-xl md:rounded-2xl bg-card border border-border/50 text-foreground/80 shadow-sm group-hover:scale-110 group-hover:bg-warmth-100 group-hover:text-warmth-700 group-hover:border-warmth-300/40 transition-all duration-soft ease-out-soft">
      ...
      {!full && (
        <Link
          href={`/${lang}/features`}
          className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-warmth-700 transition-colors duration-soft group underline decoration-warmth-300 underline-offset-4 hover:decoration-warmth-700"
        >
